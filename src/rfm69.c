...
// Even further modified by Liam Soutter
// *************************************************************************
...
// SPI interface macros and functions to map radio to the STM32 HAL
#define enable_rf_exti() HAL_NVIC_EnableIRQ(RF_EXTI_EXTI_IRQn);
#define disable_rf_exti() HAL_NVIC_DisableIRQ(RF_EXTI_EXTI_IRQn);
#define select() HAL_GPIO_WritePin(RF_CS_GPIO_Port, RF_CS_Pin, GPIO_PIN_RESET);
#define unselect() HAL_GPIO_WritePin(RF_CS_GPIO_Port, RF_CS_Pin, GPIO_PIN_SET);
#define spi_transfer_sync(dataout, datain, len) HAL_SPI_TransmitReceive(&RF_SPI, dataout, datain, len, HAL_MAX_DELAY)
#define spi_transmit_sync(dataout, len) HAL_SPI_Transmit(&RF_SPI, dataout, len, HAL_MAX_DELAY)

static inline uint8_t spi_fast_shift(uint8_t tx_data)
{
    uint8_t rx_data;
    HAL_SPI_TransmitReceive(&RF_SPI, &tx_data, &rx_data, 1, HAL_MAX_DELAY);
    return rx_data;
}
...
void send(uint8_t toAddress, const void* buffer, uint8_t bufferSize)
{
    ...
    millis_current = HAL_GetTick();
    while (!canSend() && HAL_GetTick() - millis_current < RF69_CSMA_LIMIT_MS) receiveDone();
...
{
    uint8_t data[] = {addr|0x80, value};
    select();
    spi_transmit_sync(data, 2);
    unselect();
}
...
void encrypt(const char* key) 
{
    ...
    if (key != 0)
    {
        ...
        spi_transmit_sync((uint8_t *)key, 16);
        unselect();
    }
...
uint8_t receiveDone()
{
    ...
    {
        enable_rf_exti(); // explicitly re-enable interrupts
        return 0;
    }
    receiveBegin();
    enable_rf_exti();
...
void sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize)
{
    ...
    spi_transmit_sync((uint8_t *) buffer, bufferSize);
    unselect();
